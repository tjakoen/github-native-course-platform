...
export function confirmExecute(action, word){
 return new Promise(resolve=>{
  const prev=document.activeElement;   // restore focus to the invoker on close (Escape included)
  ...
  ok.onclick=()=>{d.close();d.remove();restore();resolve(true);};
  d.querySelector("#ceNo").onclick=()=>{d.close();d.remove();restore();resolve(false);};
  d.addEventListener("cancel",()=>{d.remove();restore();resolve(false);});
  inp.focus();
 });
}
