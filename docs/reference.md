...
| `template-visibility.yml` | Flips an activity template or solution repo between public and private. Org-wide, not section-scoped. Dry-run by default. |
...
| `repo-visibility.mjs` | Releases or retires an activity by flipping its template repo public or private. Two guards, both absolute: it acts only on names that are unmistakably infrastructure (`<id>-classcode-yourname`, `<id>-solution`, `<name>-template`, `*live-demo*`), because making a student repo public is a PII incident and a typo must not be able to cause one; and making a `*-solution` repo public needs `--allow-solution-public` on top of `--execute`, because a solution repo is the worked answer. Reads the flag back after writing. Needs `Administration: write`. |
...
| `gradebook/grader-hashes.json` | Fingerprint per activity of the canonical tests its grades were computed against, so the sweep can tell "unchanged, safe to reuse" from "test was edited". `RUBRIC.md` is excluded and tracked separately under `_rubrics`, because re-grading blanks a reviewed score and a rubric edit must not cost one. Written by the sweep. |
